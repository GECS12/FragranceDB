    ...
    for product in products:
        try:
            ...
            if price_span:
                ...
                logging.warning(f"No price found for product: {original_fragrance_name}")
                price_amount = None
...
async def main():
    ...
    logging.info(f"Started scraping MassPerfumarias")
    ...
    logging.info(f"Ended scraping MassPerfumarias")

    base_path = os.path.join(os.getcwd(), 'data')
    os.makedirs(base_path, exist_ok=True)

    try:
        save_to_excel(all_fragrances, base_path, "MassPerfumarias")
    except Exception as e:
        logging.error(f"Error saving fragrances to Excel: {e}")

    end_time = time.time()
    logging.info(f"Process took {end_time - start_time:.2f} seconds.")
