            ...
            try:
                async with session.get(url) as response:
                    ...
                    for variation in variations:
                        quantity = variation['attributes']['attribute_tamanho']
                        quantity = float(re.sub(r'[^\d.]', '', quantity.replace(',', '.')))
                        price_amount = variation['display_price']
                        is_in_stock = "Available by order" if variation['backorders_allowed'] else (
...
async def scrape_category(category_url, gender):
    async with aiohttp.ClientSession() as session:
        ...
        for page in range(1, total_pages + 1):
            ...
            tasks.append(get_fragrance_links(session, page_url, gender, page))

        all_links_results = await asyncio.gather(*tasks)
        all_links = []
        ...
        all_fragrances = []
        for fragrances, (link, _, _) in zip(all_fragrances_results, all_links):
            if fragrances:
                all_fragrances.extend(fragrances)
            else:
                logging.warning(f"Empty fragrance details list found for {link}")
